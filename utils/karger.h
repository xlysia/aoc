...
// utility to find size of subset
int size(struct subset subsets[], int i){
    int xroot = find(subsets, i);
    return subsets[xroot].rank;
}
...
// minimum always
pair<int,int> kargerMinCut(struct Graph* graph)
{
    std::srand(time(NULL));
